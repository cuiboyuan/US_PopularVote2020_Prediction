---
title: "Prediction on U.S. 2020 Election Popular Vote"
...
We choose logistic regression model to estimate the outcome of the presidential election. Because we are trying to find out whether the voter will vote for Trump or Biden in the 2020 election, the outcome is binary so the logistic regression model is more appropriate and suitable in this case. 

We are using six predictor variables in our model, which are age, race, gender, income level and education level of the voters in each state. These are the representative information in both census and survey dataset. To express the model mathematically, we have:

$$ \ln(\frac{p}{1-p}) = \beta_0+\beta_1  x_{age} + \beta_2 x_{male} + \beta_3 x_{state} + \beta_4 x_{race} + \beta_5 x_{education} + \beta_6 x_{income} + \epsilon$$
...
Finally, we will fit two models, one for Trump and one for Biden, and we compare the result to predict the winner of 2020 US presidential election.
...
The result can be found in the Result section.
```{r}
...
In addition to the U.S. national popular vote, We also perform a post-stratification analysis by state. We first remove the variable state from the model and employ the same post-stratification technique with the new model. For the result, instead of two numbers of popular vote proportion in the previous analysis, we have predicted the popular vote proportion of each candidate for every state, which is illustrated by Figure 1, 2, 3 in Result section.Then we apply the winner-takes-all rule of the US election system by distributing all the state's electoral vote to the candidate with higher popular vote at that state [3]. Finally we add up all the electoral votes for each candidate. The result is illustrated in result section.
...
data.frame('Biden(%)'=biden$alp_predict*100, 'Trump(%)'=trump$alp_predict*100)
...
electoral %>% rename(Stae = state, `Trump Vote`=trump, `Biden Vote`=biden)

data.frame(Biden=electoral$biden %>% sum(), Trump=electoral$trump %>% sum())
...
We also conduct an analysis on post-stratification by state, that is, we remove the variable state from the model, and calculate the popular vote proportion of each state. The result is illustrated by graph Figure 1, 2, 3.
...
```{r}
edu <- case_when(census_data$education_level==0~"3rd Grade or less",
                     census_data$education_level==1~"Middle School - Grades 4 - 8",
                     census_data$education_level==2~"Completed some high school ",
                     census_data$education_level==3~"High school graduate",
                     census_data$education_level==4~"Other post high school vocational training",
                     census_data$education_level==5~"Completed some college, but no degree",
                     census_data$education_level==6~"Associate Degree",
                     census_data$education_level==7~"College Degree (such as B.A., B.S.)",
                     census_data$education_level==8~"Completed some graduate, but no degree",
                     census_data$education_level==9~"Masters degree",
                     census_data$education_level==10~"Doctorate degree"
                     )
df <- data.frame(Education=edu, Level=census_data$education_level) %>% group_by(Education) %>% summarise(Level = mean(Level))
df[order(df$Level),]
